...
```powershell
python .\scripts\verify_backup.py backup.kbf -o recovered\mymoney.sqlite
```
...
包含的功能（我暂时用得到的功能）：
1. 导入和导出账本，多账本命名、管理。一个账本是一个 `.sqlite` 文件。
2. 主设置界面调节浅色/深色/跟随系统。
...
5. 自定义一级二级分类、一级二级所属账户、显示外币种类（设置时加上搜索）。删除分类前要提示：删除分类也会删除其下流水，等待3秒，确认两遍。分类不可重名。修改分类名称后，涉及条目也自动修改。可以从记账/编辑界面跳转，也可以从主设置访问。
